...
export default function BookingForm({ booking, isEditMode, onSuccess }) {
  ...
  const [extendType, setExtendType] = useState("Time");
  ...
  const extendedHourlyRate = 500;
  // const extendedDays = Math,
  //Extended Fee

const extendedDateCount = (() => {
  // 1. Check if both dates are valid before trying to do math on them
  if (extendedDate instanceof Date && !isNaN(extendedDate) && checkOut instanceof Date && !isNaN(checkOut)) {
    const diffTime = extendedDate.getTime() - checkOut.getTime();
    return Math.ceil(diffTime / (1000 * 60 * 60 * 24));
  }
  // 2. Return 0 if the dates aren't set yet
  return 0;
})();



  const extendedTimeFee = (() => {
    if(isExtend !== "Yes") return 0;

    if(extendType === "Time"){
      return (parseFloat(extendedHours || 0) * extendedHourlyRate); 
    }
    return 0;
  })();

  const extendedDateFee = (() => {
    if(isExtend !== "Yes") return 0;

    if (extendType === "Date") {
    // Ensure both are valid Date objects before subtracting
    if (extendedDate instanceof Date && !isNaN(extendedDate) && checkOut instanceof Date && !isNaN(checkOut)) {
      const diffTime = extendedDate.getTime() - checkOut.getTime();
      const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
      return selectedRoom ? selectedRoom.price * (diffDays > 0 ? diffDays : 0) : 0;
    }
    return 0;
  }
  return 0;
  ...
  const requiredPayment =
    form.paymentType === "fullpayment" ? grandTotal : requiredDeposit;
  const balance =
    form.paymentType === "fullpayment"
      ? 0
      : requiredPayment - parseFloat(form.payment || 0);
    ...
    if (booking) {
      ...
      setForm({
        ...
        check_in: booking.check_in
          ? moment(booking.check_in).format("YYYY-MM-DDTHH:mm")
          : "",
        check_out: booking.check_out
          ? moment(booking.check_out).format("YYYY-MM-DDTHH:mm")
          : "",
  ...
  const bookingFieldsetTitle = isEditMode
    ? "Booking Details"
  ...
  const handleExtendTypeChange = (e) => {
    ...
    setExtendType(value);
    if(value === "Time") setExtendedDate(null);
    if(value === "Date") setExtendedHours(null);
  ...
  const handleExtendedHours = (e) => {
    const hours = e.target.value === "" ? null : parseInt(e.target.value, 10);
    setExtendedHours(hours);

    if(hours && extendedHours){
  ...
  //       date.setHours(12 + extendedHours, 0, 0, 0);
  ...
  const handleExtendedDate = (e) => {
    ...
    const date = new Date(e.target.value);
    date.setHours(12, 0, 0, 0); // default 12pm
    setExtendedDate(date);
  }
  ...
  const handleSubmit = async (action) => {
    ...
    if (!isEditMode) {
      ...
      if (action === "paynow" && paymentValue < requiredDeposit) {
        toast.warning(
          `Deposit must be at least ₱${requiredDeposit.toFixed(2)}`,
        );
        return;
      }
      ...
      try {
        ...
        const bookingData = {
          ...
          total_pax:
            Number(selectedRoom.room_capacity || 0) +
            Number(form.additionalPax || 0),
        ...
        const invoiceData = {
          ...
          breakfast_package:
            [form.breakfast1, form.breakfast2].filter(Boolean).join(", ") ||
            null,
              ...
              <div className="space-y-3">
                <div className="gap-4">
                  <div className="">
                    ...
                    <input
                      ...
                      value={
                        form.check_in
                          ? moment(form.check_in).format("YYYY-MM-DDTHH:mm")
                          : ""
                      }
                  ...
                  <div className="">
                    ...
                    <input
                      ...
                      value={
                        form.check_out
                          ? moment(form.check_out).format("YYYY-MM-DDTHH:mm")
                          : ""
                      }
                  ...
                  <div className="text-right">
                    <div className="flex justify-end items-center">
                      <span className="text-xs italic">
                        (₱500.00 per hour before 2PM)
                      </span>
                    </div>
                    <input
                      name="earlyCheckIn"
                      value={
                        earlyCheckIn instanceof Date && !isNaN(earlyCheckIn)
                          ? moment(earlyCheckIn).format("YYYY-MM-DDTHH:mm")
                          : ""
                      }
                      onChange={(e) => {
                        if (!e.target.value) {
                ...
                {isEditMode && (
                  <div className="flex gap-4">
                    ...
                    <div className="text-sm justify-items-start items-center">
                      <fieldset className="border rounded-xl p-4">
                        <legend>Extend Check-out?*</legend>
                      ...
                      {isExtend === "Yes" && (
                        <div>
                          <div>
                            <p className=" text-left font-medium pb-3">
                              Extend Type?
                            </p>
                            <label className="px-4 py-2 cursor-pointer">
                              <input
                          ...
                          <div>
                            ...
                            {extendType === "Time" && (
                              <div>
                                <label className="block text-left text-sm font-medium mb-1 pt-3">
                                  Extended Hours:
                                </label>
                                <input
                                  type="number"
                                  name="extendedHours"
                                  value={extendedHours ?? ""}
                                  onChange = {handleExtendedHours}
                                  step="1"
                                  required
                                  disabled={isExtend === "No"}
                                  min="1"
                                  placeholder="Hours (e.g. 1, 2, 3)"
                                  className="w-full border rounded py-3 text-center focus:outline-none focus:ring-2 focus:ring-blue-500
                      disabled:bg-gray-100 disabled:text-gray-500 disabled:cursor-not-allowed"
                                />
                              </div>
                            )}
                            {extendType === "Date" && (
                              <div>
                                ...
                                <input
                                  type="date"
                                  name="extendedDate"
                                  value={extendedDate instanceof Date && !isNaN(extendedDate)
                                    ? moment(extendedDate).format("YYYY-MM-DD")
                                    : ""
                                  }
                                  onChange={handleExtendedDate}
                                  required
                                  disabled={isExtend === "No"}
                                  min={checkOut instanceof Date && !isNaN(checkOut) ? moment(checkOut).format('YYYY-MM-DD') : ''}
            ...
            <fieldset className="border rounded-xl p-4">
              ...
              <div>
                ...
                <div className="text-left pt-2 ">
                  ...
                  <span>
                    {booking.breakfast_complementary
                      ? booking.breakfast_complementary
                      : " -"}
                  </span>
                </div>
                ...
                {form.breakfast_complementary === "1-complementary" && (
                  <div className="text-left">
                    ...
                    >
                      ...
                      <option value="Hotdog and Cheese">
                        Hotdog and Cheese
                      </option>
                    </select>
                  </div>
                ...
                <div className="text-sm flex justify-items-start items-center ">
                  ...
                  <label className="px-4 py-2 cursor-pointer">
                    <input
                      ...
                      disabled={isEditMode}
                      checked={
                        !form.isAddService || form.isAddService === "false"
                      }
                    />
                    <span className="pl-2">No</span>
                  </label>
          ...
          <div className="flex-1 inline-block">
              ...
              return (
                <div>
                  <fieldset className="border rounded-xl p-4 mb-2">
                    ...
                    <h1
                      className={` text-[120px] text-center pb-2 font-medium  
                ${
                  form.invoice_status === "PAID"
                    ? "text-green-500"
                    : form.invoice_status === "UNPAID"
                      ? "text-red-500"
                      : form.invoice_status === "PARTIAL"
                        ? "text-yellow-500"
                        : "text-gray-500"
                }
                `}
            ...
            <fieldset className="border rounded-xl p-4">
              ...
              <div className="w-full text-sm">
                ...
                <div className="px-2">
                  <p className="font-bold text-sm pt-4 text-left">
                  ...
                  {earlyCheckInFee > 0 && checkIn && (
                    <div className="flex py-1">
                      <p>
                        Early Check-in Fee: ({14 - checkIn.getHours()} hr/s
                        before 2PM)
                      </p>
                      <span className="text-right font-medium flex-1">
                        ₱{earlyCheckInFee.toLocaleString()}
                      </span>
                    </div>
                  )}
                    ...
                    <div>
                      {
                       ...
                       <div className="flex py-1">
                      <p>
                        Extended Time: ({extendedHours} hr/s
                        ({extendedHourlyRate} hourly rate)
                        )
                      </p>
                      <span className="text-right font-medium flex-1">
                        ₱{extendedTimeFee.toLocaleString()}
                      </span>
                    </div>
                        )
                      }
                      ...
                      <p>
                        {/* aayusin */}
                        Extended Date: ({extendedDateCount ? extendedDateCount : "0"} day/s {selectedRoom.price} 
                        )
                      </p>
                      <span className="text-right font-medium flex-1">
                        ₱{extendedDateFee}
