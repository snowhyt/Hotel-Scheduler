...
export default function Rooms() {
  ...
  // const [searchRoomStatus, setSearchRoomStatus] = useState("");
...
const filteredRooms = rooms.filter((r) => {
  ...
  const matchRoomNumber =
  r.room_number.includes(search) || false;

  // Filter by room type
  // const matchRoomType =
  //   searchRoomType ? r.room_type === searchRoomType : true;

  const matchRoomType = r.room_type?.toLowerCase().includes(search.toLowerCase()) || false;

  // Filter by room status
  const matchRoomStatus = r.is_active?.toLowerCase().includes(search.toLowerCase()) || false;

  return matchRoomNumber || matchRoomType || matchRoomStatus;
});
            ...
            <tbody>
              {filteredRooms.map((room) => (
                <tr key={room.id} className="border-t">
                  ...
                  <td className={
                    `p-2
                     ${room.is_active === "available"
                    ? "text-green-700"
                    : room.is_active === "Occupied"
                    ? "text-blue-700"
                    : room.is_active === "cleaning"
                    ? "text-yellow-700"
                    :room.is_active === "inspection"
                    ? "text-purple-700"
                    :room.is_active === "not_available"
                    ? "text-red-700"
                    :room.is_active === "tenant"
                    ? "text-slate-700"
                    : room.is_active === "under_maintenance"
                    ? "text-orange-700"
                    : "text-gray-700"
                  }`
                  }>{room.is_active
                  ...
                  <td className="p-2 text-left text-gray-400">{room.description}</td>
